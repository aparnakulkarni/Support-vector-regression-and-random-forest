#-------------------------------------------------------------------------------
# Name:     ilwisRasterIO
# Purpose:  import a multiband raster to a 2D numpy array
#           export a numpy array to a raster dataset

#   TODO: check line 29-38 for southern projections
#import sys
...
def rasterCoverageToIndexedNumpy(raster,epsgcode, maxbands=100, bandLocation='bycolumn', nodata=-1e+308, skip=[]):
    ...
    try:
        ...
        if maxbands<=nbands: nbands=maxbands
        print ('We are processing '+str(nbands)+' bands')
        properties['nbands']=nbands  #store the number of bands
        ...
        for band in range(1,nbands):
            if (band+1) in skip:
                np.fromiter(it, np.float, it.box().size().xsize * it.box().size().ysize)
                continue
...
if __name__ == '__main__':
    ...
    def importSingleBand():
        try:
            ...
            fileIO.saveObject('C:/xxx/new/EV_2003_single',a)
            print ('loading object from disk')
            c=fileIO.loadObject('C:/xxx/new/EV_2003_single')
            return a,c
    ...
    def importSingleBandRaster():
        try:
            ...
            print ('saving object to disk')
            fileIO.saveObject('C:/xxx/EV_2003_al_warp_1',b)
            print ('loading object from disk')
            c=fileIO.loadObject('C:/xxx/EV_2003_al_warp_1')
            return b,c
    ...
    #raster=exportRaster()
    #storeRaster(raster[1], outputdir='C:/xxx/new', rastername='newraster', form='GTiff')
    #raster[1].store('file:///C:/xxx/new/newraster', 'GTiff', 'gdal')
